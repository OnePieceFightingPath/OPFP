...
function _buildEditorHtml(editorId, opts) {
  ...
  var cancelHtml = opts.showCancel
    ...
    : '';
  var EMOJIS = ['😀','😂','😅','🥰','😍','🤔','😢','😡','😎','🥲','👍','👎','❤️','🔥','⭐','🎉','✨','💯','🙏','👏','😤','😭','🫡','😴','🤮','👀','💀','🤣','🫶','🥳'];
  var emojiItems = EMOJIS.map(function(e){ return '<span class="evt-editor-emoji-item">' + e + '</span>'; }).join('');
  return [
    ...
    '      <button type="button" class="evt-editor-tool evt-editor-color-btn" data-editor-id="' + editorId + '" title="글자색"><span class="evt-editor-color-preview" id="color-preview-' + editorId + '" style="background:#e74c3c"></span>A</button>',
    '      <input type="color" class="evt-editor-color-picker" id="color-picker-' + editorId + '" value="#e74c3c" tabindex="-1">',
    '    </span>',
    ...
    '      <div class="evt-editor-emoji-picker" id="emoji-picker-' + editorId + '" style="display:none">' + emojiItems + '</div>',
    '    </span>',
...
function _initEditor(editorId, opts) {
  ...
  if (emojiBtn && emojiPicker) {
    ...
    emojiPicker.querySelectorAll('.evt-editor-emoji-item').forEach(function(item) {
      item.addEventListener('mousedown', function(e) {
        e.preventDefault();
        restoreRange();
